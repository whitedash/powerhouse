...
| Bot abuse of public embed endpoints (form spam; plan checkout-init, where each accepted hit costs a Stripe API call) | Layered per surface: honeypot (`_hp`, silent fake-accept) + per-IP-per-slug `RateLimiter` + route throttles on forms; the plans checkout-init adds **server-verified Cloudflare Turnstile** (`turnstilePasses()`, the /support pattern — fail-closed on missing/invalid token and on network failure via uncaught `ConnectionException`; `TURNSTILE_BYPASS` must stay false in production). Amounts are always server-derived from the catalogue — the client never supplies a price. |
| Raw CSS injection into public embeds (stored-XSS-adjacent: theme `custom_css` renders on third-party sites) | Read- AND write-gated behind dedicated super-admin-tier permissions, one per surface — `forms.custom_css` (form themes) and `products.custom_css` (plan themes), independently grantable. Non-holders never receive the stored value in page props, writes are stripped server-side (`FormThemeService`/`PlanThemeService`), and a staff edit preserves rather than wipes an admin's CSS. Injected only into the widget's shadow root, after the variable styles. |
