<script setup>
/**
 * The three customer asset sections (Websites · Domains · Services), rendered
 * inside both the portal desktop and mobile shells. Read-only; the only action
 * is the SaaS SSO launch on a service. Receives already-safe DTOs from
 * Portal\AssetController — it adds no fields of its own.
 */
import { ref } from 'vue';
...
/* ─── SaaS SSO launch (only shown when there's a real target) ─── */
        ...
        <section class="asset-sec">
            ...
            <div v-else class="asset-grid">
                <div v-for="s in services" :key="s.id" class="card card-pad asset-card">
                    ...
                    <!-- Open only when the product has a real SSO target. -->
...
.asset-sec:first-child { margin-top: 0; }
